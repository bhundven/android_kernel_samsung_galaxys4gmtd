...
void mxt224_ta_probe(int ta_status)
{
	...
	if(ta_status_pre == ta_status){
		printk("mxt224 same status\n");
		return;
	}
	...
	if (copy_data->family_id==0x81) {
		...
		{	
			if (noise_freq_table.fherr_setting) {
				...
				value  = 40;
				write_mem(copy_data, obj_address+7, size_one, &value);


				value  = 46;
				write_mem(copy_data, obj_address+13, size_one, &value);

				
				ret = get_object_info(copy_data, SPT_CTECONFIG_T28, &size_one, &obj_address);
				size_one = 1;
				value  = 19;
				write_mem(copy_data, obj_address+4, size_one, &value);
				...
				value  = 30;
				write_mem(copy_data, obj_address+8, size_one, &value);
